...
def normal_eval(preds, gold):
    ...
    print(classification_report(oh_gold, oh_pred, target_names=target_names))
...
def predict(dataset, model, batch_size):
    ...
    for i in tqdm.tqdm(range(0, len(dataset["dev"]), batch_size)):
        ...
        outputs = model.generate(input_ids=input_ids, do_sample=False, max_length=50)
...
if __name__ == '__main__':
    if args.test:
        ...
        dev_data = pd.read_csv('MTS-Dialog-TestSet-2-MEDIQA-Sum-2023.csv')

    dev_data.fillna(value={'text':''}, inplace=True)

    # dev_text = dev_data['text'].tolist()
    dev_text = dev_data['section_text'].tolist()
    ...
    quantization_config = BitsAndBytesConfig(is_quantized=True, n_bits=8)
    # reloaded_model = AutoModelForSeq2SeqLM.from_pretrained(config.base_model_name_or_path,  load_in_8bit=True,  device_map={"":0})
    reloaded_model = AutoModelForSeq2SeqLM.from_pretrained(config.base_model_name_or_path, quantization_config=quantization_config, device_map={"": 0})
    # Load the Lora model
    ...
    # Validate params list before using it
    if len(params) >= 5:
        param_dict = {'model': params[0], 'task': params[1], 'train_data': params[2], 'undersample': params[3], 'synthetic_data': params[4]}
    else:
        # Fallback or default assignment if params list is not as expected
        param_dict = {'model': 'unknown', 'task': 'unknown', 'train_data': 'unknown', 'undersample': 'unknown', 'synthetic_data': 'unknown'}
